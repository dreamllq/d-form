import type { FieldSchema, FieldState, FieldMeta, ValidationResult } from '@d-form/shared'
...
export class Field {
  ...
  setValue(value: any): void {
    ...
    if (this.meta.schema?.validation?.trigger === 'change') {
      this.validate()
    }
  }
  ...
  setTouched(touched: boolean): void {
    ...
    if (touched && this.meta.schema?.validation?.trigger === 'blur') {
      this.validate()
    }
  }
  ...
  validate(): ValidationResult | Promise<ValidationResult> {
    const rules = this.meta.schema?.validation?.rules || []
  ...
  private _validateRule(rule: import('@d-form/shared').ValidationRule, value: any, fieldName: string): string | undefined {
    switch (rule.type) {
      case 'required':
  ...
  private async _validateAsync(): Promise<ValidationResult> {
    this._state.validating = true
    this._emit('stateChange', this._state)
    
    try {
      const result = await validateFieldAsync(this, this.meta.name)
