...
function mountAdapter(props: Record<string, any>) {
  return mount(SwitchAdapter, {
    props: { name: 'test', ...props },
    global: { stubs: { ElSwitch: ElSwitchStub } },
  })
}
...
describe('SwitchAdapter', () => {
  it('renders without error', () => {
    ...
    expect(wrapper.findComponent(ElSwitchStub).exists()).toBe(true)
  })

  it('passes modelValue', () => {
    const wrapper = mountAdapter({ modelValue: true })
    expect(wrapper.findComponent(ElSwitchStub).props('modelValue')).toBe(true)
  })

  it('passes disabled', () => {
    const wrapper = mountAdapter({ modelValue: false, disabled: true })
    expect(wrapper.findComponent(ElSwitchStub).props('disabled')).toBe(true)
  })
  ...
  it('emits update:modelValue', async () => {
    const wrapper = mountAdapter({ modelValue: false })
    const stub = wrapper.findComponent(ElSwitchStub)
  ...
  it('passes componentProps via v-bind', () => {
    ...
    const stub = wrapper.findComponent(ElSwitchStub)
    expect(stub.props('activeText')).toBe('On')
    expect(stub.props('inactiveText')).toBe('Off')
  })
})
